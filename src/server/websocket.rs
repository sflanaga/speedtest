...
pub const MAX_DURATION_MS: u64 = 300_000;       // 5 minutes
pub const MAX_BYTES: u64 = 10_737_418_240;      // 10 GB
pub const MAX_CHUNK_BYTES: usize = 1_048_576;   // 1 MB
pub const MAX_PING_COUNT: u64 = 5_000;          // 5,000 pings max
pub const MIN_CHUNK_BYTES: usize = 64;          // 64 bytes minimum
...
pub struct AppState {
    pub cfg: ServerOpts,
    pub chunk_buf: Arc<Vec<u8>>,
}

fn validate_control_msg(ctrl: &ControlMsg, cfg: &ServerOpts) -> Option<ValidationError> {
    match ctrl.phase {
        Phase::Ping => {
...
pub async fn run_server(cfg: ServerOpts) {
...
// [Rest of the server code continues - handle_socket, handle_ping_echo, download_phase, etc.]
                ...
                match ctrl.phase {
                    Phase::Ping => {
                        if ctrl.action.as_deref() == Some("start") {
                            ...
                            let test_id = ctrl.test_id.unwrap_or(0);
                            let ping_count_val = ctrl.ping_count.unwrap_or(0);
                            ...
                            {
                                let mut test = current_test.lock().await;
                                test.ping_count = ping_count_val;
                                test.ping_avg_ms = ping_avg;
                                test.ping_min_ms = ping_min;
                                test.ping_max_ms = ping_max;
                            }
                            info!(%conn_id, %addr, test_id, ping_count_val, ping_avg_ms=format!("{:.2}", ping_avg), ping_min_ms=format!("{:.2}", ping_min), ping_max_ms=format!("{:.2}", ping_max), "ping done");
                        } else {
